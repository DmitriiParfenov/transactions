...
def operations_db(path):
    """
    Функция проверяет корректность данных в json-файле и сам файл.
    Функция в качестве аргумента получает путь к json-файлу с банковскими операциями.
    Если файл пустой, то функция вернет оповещающую строку о необходимости проверки файла.
    Если в файле некорректные данные, то функция возбудит исключение ValueError.
    Функция проверяет корректность ключей в файле, подразумевается, что если 'state' = 'EXECUTED', то файл
    содержит корректные данные.
    Функция возвращает список с отсортированными в порядке убывания банковскими операциями в размере
    до 5 элементов
    :param path: str
    :return: list
    """
